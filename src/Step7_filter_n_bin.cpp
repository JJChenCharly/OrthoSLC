...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (std::string(argv[i]) == "--help" || std::string(argv[i]) == "-h")
        {
            ...
            std::cout << "Usage: Step7_filter_n_bin -i input/ -o output/ -s seq_len_info.txt [options...]\n\n";
            ...
            std::cout << "  -p or --pre_cluster_path -> path/to/output/pre_cluster.txt\n";
    ...
    if (!(fs::exists(q_bin_op))) {
        ...
        std::cerr << "Error: path provided to '-s or --seq_len_path' do not exist. 路径不存在\n";
        exit(0);
    } else if (!(fs::exists(pre_cluster_path))) {
        std::cerr << "Error: path provided to '-p or --pre_cluster_path' do not exist. 路径不存在\n";
        exit(0);
    }
        ...
        pool.enqueue([in_p, bin_op, seq_len_, b_level, r_limit, bin_spe_, lock_or_not, LOCK] { // regradless of number of input parameter
            ...
            for (const auto& [qs, nested_um] : q_s_d) {
                ...
                for (const auto& [sub, its_score] : nested_um) {
                    ...
                    if (qs.substr(0, 5) == s_spe) {
                        continue;
                    } 
                    else if (bin_spe_->at(qs).find(s_spe) != bin_spe_->at(qs).end()) { // if qs cluster has that spe
                        continue;
                    } else {

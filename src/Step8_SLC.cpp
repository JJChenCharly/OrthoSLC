...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (std::string(argv[i]) == "--help" || std::string(argv[i]) == "-h")
        {
            ...
            std::cout << "Usage: Step8_SLC -i input/ -o output/ [options...]\n\n";
            std::cout << "options: " << "\n";
            std::cout << "  -i or --input_path --------> <dir> path/to/input/directory from Step 7\n";
            std::cout << "  -o or --output_path -------> <dir> path/to/output/directory\n";
            std::cout << "  -u or --thread_number -----> <int> thread number, default: 1\n";
            std::cout << "  -p or --pre_cluster_path --> <txt> path/to/output/pre_clustered file from Step 3\n";
            std::cout << "  -S or --compression_size --> <int> compression size, default: 10, 'all' means one-step-to-final\n";
    ...
    fs::path parent_out_dir = fs::path(out_dir).parent_path();
    if (!(fs::exists(parent_out_dir))) {

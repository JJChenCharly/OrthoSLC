...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (std::string(argv[i]) == "--help" || std::string(argv[i]) == "-h")
        {
            ...
            std::cout << "  -d or --concatenated_fasta -> path/to/output/dereplicated concatenated FASTA\n";
    ...
    if (!(fs::exists(parent_dereped_cated_fasta_pth))) {
        std::cerr << "Error: parent path provided to '-d or --concatenated_fasta' do not exist. 路径不存在\n";
        exit(0);
    }
    else if (!(fs::exists(parent_nr_dir))) {
        std::cerr << "Error: parent path provided to '-n or --nr_genomes' do not exist. 路径不存在\n";
        exit(0);
    }
    else if (!(fs::exists(parent_len_info_path))) {
        std::cerr << "Error: parent path provided to '-l or --seq_len_info' do not exist. 路径不存在\n";
        exit(0);
    }
    else if (!(fs::exists(parent_pre_cluster_path))) {
        std::cerr << "Error: parent path provided to '-p or --pre_cluster' do not exist. 路径不存在\n";
        exit(0);
    }

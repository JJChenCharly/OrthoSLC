...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (std::string(argv[i]) == "--help" || std::string(argv[i]) == "-h")
        {
            ...
            std::cout << "Usage: Step7_RBF -i input/ -o output/ [options...]\n\n";
            std::cout << "  -i or --input_path -----> <dir> path/to/input/directory from Step 6\n";
    ...
    fs::path parent_RBB_op_dir_pth = fs::path(RBB_op_dir_pth).parent_path();
    if (!(fs::exists(parent_RBB_op_dir_pth))) {

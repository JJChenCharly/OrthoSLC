...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (std::string(argv[i]) == "--length_limit" || std::string(argv[i]) == "-r")
        {
            ...
            if((r_ < 0) || (r_ > 1)) {
                std::cerr << "-r or --length_limit must be between 0 < r <= 100";
                exit(0);
            }
        }
        else if (std::string(argv[i]) == "--similarity_limit" || std::string(argv[i]) == "-m")
        {
            m_ = std::stod(argv[i + 1]);
            if((r_ < 0) || (r_ > 100)) {
                std::cerr << "-m or --similarity_limit must be between 0 < r <= 100";
                exit(0);
            }
        }
        else if (std::string(argv[i]) == "--weight" || std::string(argv[i]) == "-w")
        {
            weight_arg = argv[i + 1];
        }
        else if (std::string(argv[i]) == "--help" || std::string(argv[i]) == "-h")
        {
            ...
            std::cout << "  -r or --length_limit ------> <float> length difference limit, 0 < r <= 1, default: 0.8\n";
            std::cout << "  -m or --similarity_limit --> <float> similarity limit, 0 < r <= 100, default: 80\n";
            std::cout << "  -w or --weight ------------> <str> select from <'none', 'pident', 'bitscore', 'evalue'> for clustering, \n";
    ...
    int weight = 3;
    std::unordered_map<std::string, int> wei;
    wei["bitscore"] = 0;
    wei["pident"] = 1;
    wei["evalue"] = 2;
    if (weight_arg != "none") {
        weight = wei[weight_arg];
    }
    ...
    for(int i = 0; i < missions.size(); ++i) {
        ...
        futures.emplace_back(
            pool.enqueue([in_p, bin_op, seq_len_, b_level, r_limit, m_limit, &weight, bin_spe_, lock_or_not, LOCK] { // regradless of number of input parameter
                    
                std::ifstream a_qb_op(*in_p);

                std::unordered_map<std::string, std::unordered_map<std::string, std::array<double,3>>> q_s_d;
                std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> sspe_m;
                
                std::string a_hit;
                std::string query, subject, identity, score, evalue;
                ...
                while (std::getline(a_qb_op, a_hit)) {
                    ...
                    std::getline(ss_, identity, '\t');
                    double identity_ = stod(identity);
                    if (identity_ < m_limit) continue;
                    
                    std::getline(ss_, score, '\t');
                    std::getline(ss_, evalue);

                    // if (query.substr(0, query.find("-")) == subject.substr(0, subject.find("-"))) {
                    //     continue;
                    // } else {
                    double score_ = stod(score);
                    double evalue_ = stod(evalue);
                    q_s_d[query][subject] = {score_, identity_, evalue_};
                        ...
                        if (bin_spe_->at(qs).find(s_spe) != bin_spe_->at(qs).end()) { // if qs pre cluster has that spe
                            ...
                            if (sspe_m.at(qs).at(s_spe).size() >  1) { // if multiple hit
                                for (const auto &id_ : sspe_m.at(qs).at(s_spe)) {
                                    ...
                                    if (its_score[0] < q_s_d[qs][id_][0]) { // if a better hit
                                        best_hit = false;
                                        break;
                                    } else if (its_score[0] == q_s_d[qs][id_][0]) {
                            ...
                            if (best_hit) {
                                ...
                                if (c1 || c2) { // if len too diff
                                    ...
                                    if (weight == 3) {
                                        ...
                                        bins_to_save[b].push_back(p_to_s + "\t" + std::to_string(-std::log10(its_score[weight]+1)) + "\n");
                                    } else {
                                        bins_to_save[b].push_back(p_to_s + "\t" + std::to_string(its_score[weight]) + "\n");
                                    }
                                }
                            }
                        }
                ...
                // saving ----
                bool use_lock =  lock_or_not;

                // helper to save one bin
                auto save_bin = [&](int bin_id, const std::vector<std::string>& items) {
                    ...
                    auto outp    = fs::path(*bin_op) / fname;
                    ...
                    if (use_lock) {
                        ...
                        for (auto const &line : items)
                            ofs << line;
                        // ofs.closed by destructor, then lock released
                    } else {
                        std::ofstream ofs(outp, std::ios::app);
                        for (auto const &line : items)
                            ofs << line;
                        // ofs.closed by destructor
                    }
                };

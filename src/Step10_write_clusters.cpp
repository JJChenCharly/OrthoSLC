...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (std::string(argv[i]) == "--help" || std::string(argv[i]) == "-h")
        {
            ...
            std::cout << "Usage: Step10_write_clusters -i input_path -o output/ -f concatenated.fasta [options...]\n\n";
            std::cout << "options:\n";
    ...
    else if (!(fs::exists(cated_fasta_path))) {
        std::cerr << "Error: path provided to '-f or --fasta_path' do not exist. 路径不存在\n";
        exit(0);
    }
